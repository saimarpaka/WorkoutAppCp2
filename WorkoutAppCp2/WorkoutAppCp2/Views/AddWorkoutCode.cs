...
{
    public class AddWorkoutCode : ContentView
    {       
        ...
        public List<Button> listAddExerciseBtns = new List<Button>();
        ...
        {
            Grid cardGrid = CreateDayGrid((WeekCounter).ToString(), DayCounter.ToString());
            ...
            listAddExerciseBtns.Add(btnAddExercise);
        ...
        private void BtnAddDay_Clicked(object sender, System.EventArgs e)
        {
            ...
            StackLayout DayCard = new StackLayout();
            Label WeekLabel = null;
            WeekLabel = (Label)listWeekCards[buttonIndex].Children.First();

            Grid dayGrid = CreateDayGrid(WeekLabel.Text[WeekLabel.Text.Length - 1].ToString(), (dictDayCardGridCounter[(Button)sender] + 1).ToString());
            Button btnAddExercise = new Button
            {
                Text = "Add",
                FontSize = Device.GetNamedSize(NamedSize.Micro, typeof(Label)),
            ...
            btnAddExercise.Clicked += BtnAddExercise_Clicked;
            listAddExerciseBtns.Add(btnAddExercise);
            listDayCardGrid.Add(dayGrid);

            DayCard.Children.Add(new Label { Text = "Day " + (dictDayCardGridCounter[(Button)sender] + 1), FontSize = Device.GetNamedSize(NamedSize.Medium, typeof(Label)) });
            DayCard.Children.Add(dayGrid);
            DayCard.Children.Add(btnAddExercise);
            dictDayCardGridCounter[(Button)sender] += 1;
            listWeekCards[buttonIndex].Children.Insert(listWeekCards[buttonIndex].Children.IndexOf((Button)sender), new CardView { Content = DayCard, BackgroundColor = Color.FromHex("#f2f4f5"), HasShadow = true });
            ScrollTo((Button)sender);
        }

        private void BtnAddExercise_Clicked(object sender, System.EventArgs e)
        {
            int buttonIndex = listAddExerciseBtns.IndexOf((Button)sender);
        ...
        private Grid CreateDayGrid(string _weekCounter, string _dayCounter)
        {
